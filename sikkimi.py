...
DAILY_LIMIT = _parse_daily_limit(os.getenv("DAILY_LIMIT", "3000"))
...
# ... (omitting duplicate simple DB methods, assuming standard usage from previous code)
...
# Routing all text/photo based on STATE
async def central_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == STATE_WALLET_RECEIPT and msg.photo:
    ...
    if state == STATE_EDIT_PKG_PRICE and _is_admin(user.id) and msg.text:
        try:
            ...
            context.user_data.clear()
            await msg.reply_text("قیمت با موفقیت آپدیت شد ✅")
        except:
...
async def wallet_admin_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("wal_ok"):
        ...
        try:
            await context.bot.send_message(req[0], f"مبلغ {_format_toman(req[1])} با موفقیت به کیف پول شما اضافه شد ✅")
        except: pass
    else:
        await asyncio.to_thread(update_wallet_req, req_id, 'rejected')
        await query.edit_message_caption(caption=query.message.caption + "\n\nرد شد ❌")
        try:
            await context.bot.send_message(req[0], f"درخواست شارژ کیف پول شما رد شد ❌")
        except: pass
...
# --- Reservation Payment Overrides (Wallet Intergration) ---
# Note: Re-using the reservation slots logic but updating checkout.

async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    if text == "پنل مدیریت 🛠" and _is_admin(update.effective_user.id):
        await update.message.reply_text("پنل مدیریت باز شد:", reply_markup=_admin_panel_keyboard())
    elif text == "⚙️ تنظیمات و ویرایش" and _is_admin(update.effective_user.id):
        await update.message.reply_text("بخش تنظیمات:", reply_markup=_settings_menu_keyboard())
    elif text == "📝 ویرایش خوش امدگویی":
    ...
    elif text == "بازگشت" or text == "🔙 بازگشت به پنل":
        ...
        # Pass to central router for states
        await central_router(update, context)

async def reserve_slots_custom(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Dynamic logic for choosing slots based on `packages` table is required when clicking on package sizes.
    query = update.callback_query
    if query.data.startswith(CB_SLOT_PREFIX):
        ...
        # Show final checkout, allow wallet if balance sufficient
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(check_membership_gate, pattern="^check_join$"))
    app.add_handler(CallbackQueryHandler(ref_actions, pattern="^ref_"))
    app.add_handler(CallbackQueryHandler(wallet_charge_init, pattern="^wallet_charge$"))
    app.add_handler(CallbackQueryHandler(wallet_paid_callback, pattern="^wallet_paid$"))
    app.add_handler(CallbackQueryHandler(wallet_admin_callbacks, pattern="^wal_"))
    app.add_handler(CallbackQueryHandler(admin_edits_callback, pattern="^edit_"))
    app.add_handler(CallbackQueryHandler(inventory_callback, pattern="^pkg_"))
    
    # Stats Pagination
    app.add_handler(CallbackQueryHandler(lambda u,c: show_admin_stats(u,c, int(u.callback_query.data.split("|")[1])), pattern="^adm_pg\|"))
    app.add_handler(CallbackQueryHandler(admin_user_profile, pattern="^adm_usr\|"))
    
    # Reservation Interceptors
    app.add_handler(CallbackQueryHandler(reserve_slots_custom, pattern=f"^{re.escape(CB_SLOT_PREFIX)}|^{re.escape(CB_PACKAGE_PREFIX)}|^payw_"))
