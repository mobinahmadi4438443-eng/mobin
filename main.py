...
# ==========================================
# 🧩 سیستم هوشمند ایموجی و پروگرس بار (ضدگلوله)
# ==========================================
...
def parse_emojis(text: str) -> str:
    # این فرمول اجازه نمی‌دهد تگ‌های از قبل ساخته شده دوباره تغییر کنند (رفع باگ کرش کردن ربات)
...
# سیستم امن برای ساخت دکمه‌ها تا در صورت خراب بودن آیدی ایموجی ربات کرش نکند
...
async def get_battle_arena_text(user_id: int):
    ...
    # پروگرس‌بارها با رنگ‌های اختصاصی شما
    ...
    # انتخاب یک انیمه به صورت رندوم برای گزارش لایو نبرد
    ANIME_EMOJIS = [
    ...
    text = (
        ...
        f" {random_anime} <i>دراخور با چشمانی خونین به تو خیره شده است... منتظر حرکت توست مبارز!</i> "
    )
...
# ==========================================
# 🧩 ساختار ایمن کیبوردها (جلوگیری از باگ دکمه)
# ==========================================
...
async def get_raw_character_keyboard(user_id: int):
    ...
    for i in range(1, 5):
        name = await get_setting(f"char{i}_name", f"کاراکتر {i}")
        b = await btn(name, f"btn_selectchar_{i}_{user_id}", f"char{i}_emoji", "")
        chars.append(b)
    return [
        [chars[0], chars[1]],
        [chars[2], chars[3]],
...
async def handle_all_buttons(callback: types.CallbackQuery):
    ...
    if action == "close":
        ...
        if is_purchased: pet_text += "\n\nخریداری شده 6028565819225542441"
        else: pet_text += "\n\nخریداری نشده 6032606743500951856"

        kb = []
        if is_purchased: kb.append([await btn("فعال کردن برای نبرد", f"btn_activatepet_{char_id}_{owner_id}", "فعال کردن برای نبرد", "5780857858216173092")])
        else: 
        ...
        await transition_menu(callback, f"photo_pet_{char_id}", parse_emojis(pet_text), kb)
        return await callback.answer()
