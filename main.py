...
def parse_emojis(text: str) -> str:
    # اعداد 18 تا 22 رقمی را به ایموجی پرمیوم تبدیل می‌کند (تداخلی با شماره کارت 16 رقمی ندارد)
    return re.sub(r'(?<!\d)(\d{18,22})(?!\d)', r'<tg-emoji emoji-id="\1">✨</tg-emoji>', text)

# تابع ساخت پروگرس بار متحرک با کدهای ایموجی پرمیوم
...
# ⚔️ تابع تولید متن زنده و هوشمند میدان نبرد
async def get_battle_arena_text(user_id: int):
    ...
    # آیدی ایموجی‌های پرمیوم شما
    EMOJI_RED_LINE = "5868376419691663420"     # خط قرمز (خون)
    EMOJI_BLACK_LINE = "5870807534389957123"   # خط مشکی (خالی)
    EMOJI_BLUE_LINE = "5868656266875769200"    # خط آبی (شیلد)
    EMOJI_ORANGE_LINE = "5868587719197724849"  # خط نارنجی (دشمن)
    ...
    text = (
        f"6021608144004716962 <b>میدان نبرد: تارتاروس (مرحله ۱)</b> 6021608144004716962\n\n"
        f"6021493696011180326 <b>دشمن: دراخور</b> (باس اول)\n"
        f"5780382873487939194 جان:\n{boss_bar} <code>{boss_hp:,} / {boss_max:,}</code>\n"
        f"5453991094435997597 قدرت حمله: <code>100 - 300 دمیج</code>\n\n"
        
        f"5987865893084861885 <b>وضعیت شما:</b>\n"
        f"5868278765020255710 خون:\n{hp_bar} <code>{player_hp} / {player_max}</code>\n"
        f"6028551194861899805 شیلد:\n{shield_bar} <code>{player_shield} / {shield_max}</code>\n\n"
        
        f"5453991094435997597 سلاح: <code>چاقو (لول ۱)</code> | 5453991094435997597 دمیج: <code>150</code>\n"
        f"6032699501909644905 سکه: <code>{coins:,}</code> | 5213094908608392768 طلا: <code>{gold}</code>\n\n"
        
        f"5282843764451195532 <b>گزارش نبرد:</b>\n"
        f"6037584997144074766 <i>دراخور با چشمانی خونین به تو خیره شده است... منتظر حرکت توست مبارز!</i> 5780382873487939194"
    )
...
async def get_raw_battle_arena_keyboard(user_id: int):
    return [
        [{"text": "دمیج دادن", "callback_data": f"btn_action_damage_{user_id}", "icon_custom_emoji_id": await get_emoji("دمیج دادن", "5958322028531423656")},
         {"text": "دمیج حیوان", "callback_data": f"btn_action_petdmg_{user_id}", "icon_custom_emoji_id": await get_emoji("دمیج حیوان", "6042051380879822629")}],
        [{"text": "قدرت", "callback_data": f"btn_action_power_{user_id}", "icon_custom_emoji_id": await get_emoji("قدرت", "6037163978679916501")},
        ...
        [{"text": "خرید HP", "callback_data": f"btn_action_buyhp_{user_id}", "icon_custom_emoji_id": await get_emoji("خرید HP", "6032699501909644905")}],
        [{"text": "بازگشت به منو داستانی", "callback_data": f"btn_backgamemenu_{user_id}", "icon_custom_emoji_id": await get_emoji("بازگشت", "6041794945562451034")}]
    ]
